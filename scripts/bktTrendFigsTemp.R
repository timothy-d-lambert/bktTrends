...
outFiles<-paste0("results/realTrendOutTemp",c(0,1),".rds")
...
trendScale<-seq(-0.88,0.5,length.out=grains)
...
png.par("figures/bktTrendContoursTemp.png",mfcol=c(3,2),
...
for(ag in 0:1){
  ...
  for(g in 1:3){ 
    ...
    el<- seq(15,20.5,length.out=grains)
    wa<- seq(0.26,3.59,length.out=grains)
    
    e<-(el-attributes(scale(trendOut$siteTemp))$'scaled:center')/
      attributes(scale(trendOut$siteTemp))$'scaled:scale'
    ...
    for(i in 1:grains){
      for(j in 1:grains){
        predTrend[i,j,]<-out$sims.list$trendMu[,g]+
          ...
          a[i]*e[j]*out$sims.list$betaTrendElevArea
      }
    }
    ...
    title(ylab="Site Temperature Index",line=2.5)
    ...
    hull<-trendOut[majGeol==mg][
      chull(trendOut[majGeol==mg,siteTemp],
            trendOut[majGeol==mg,log(wsArea)])]
    hullTemp<-hullArea<-NULL
    for(h in 1:nrow(hull)){
      hullTemp[h]<-which.min(abs(hull$siteTemp[h]-el))
      hullArea[h]<-which.min(abs(log(hull$wsArea[h])-wa))
    }
    
    polygon(c(par("usr")[2],log(hull$wsArea),log(hull$wsArea[1]),par("usr")[c(2,2,1,1,2)]),
            c(par("usr")[4],hull$siteTemp,hull$siteTemp[1],par("usr")[c(4,3,3,4,4)]),
            border=NA,col='gray')
    points(siteTemp~log(wsArea),data=trendOut[majGeol==mg],pch=ifelse(endsIn0,1,19),cex=1,col=gray(0.3))
    ...
    if(ag==0&g==1){
      ...
      text(3.2,19+which.min(abs(trendScale-log(0.9)/diff(jagsData$year)[1])),"-10%")
      text(3.2,19+which.min(abs(trendScale-log(0.95)/diff(jagsData$year)[1])),"-5%")
      text(3.2,19+which.min(abs(trendScale))/grains*100,"0%")
      text(3.2,19+which.min(abs(trendScale-log(1.05)/diff(jagsData$year)[1])),"5%")
    }
...
# tiff.par("figures/parameterValuesGeoCat.tif",mar=c(2.5,4.5,1,1),width=3.75,height=5)
# 
