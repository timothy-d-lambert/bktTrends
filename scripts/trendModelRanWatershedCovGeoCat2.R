...
model{
  ...
  # mu.star ~ dnorm(0, 0.01)      # overall intercept
    ...
    betaBasa ~ dnorm(0, 0.01) #basaltic diff from sili
    betaGran ~ dnorm(0, 0.01) #granitic diff from sili
  ...
  #betas for mean abundance
  betaElev~dnorm(0,0.01)
  betaWsArea~dnorm(0,0.01)
  betaElevArea~dnorm(0,0.01)
  betaFlood~dnorm(0,0.01)
  betaElevBasa~dnorm(0,0.01)
  betaElevGran~dnorm(0,0.01)
  betaWsAreaBasa~dnorm(0,0.01)
  betaWsAreaGran~dnorm(0,0.01)
  
  #two way interactions for trend
  betaTrendElev~dnorm(0,0.01)
  betaTrendWsArea~dnorm(0,0.01)
  betaTrendFlood~dnorm(0,0.01)
  betaTrendBasa ~ dnorm(0, 0.01) 
  betaTrendGran ~ dnorm(0, 0.01)
  
  #three way interactions for trend
  betaTrendElevBasa~dnorm(0,0.01)
  betaTrendElevGran~dnorm(0,0.01)
  betaTrendWsAreaBasa~dnorm(0,0.01)
  betaTrendWsAreaGran~dnorm(0,0.01)
  
  for(i in 1:nSites){
    siteEffect[i]~dnorm(mu.star+betaBasa*basa[i]+betaGran*gran[i]+
                          (betaElev+betaElevBasa*basa[i]+betaElevGran*gran[i])*elev[i]+
                          (betaWsArea+betaWsAreaBasa*basa[i]+betaWsAreaGran*gran[i])*wsArea[i]+
                          betaElevArea*elev[i]*wsArea[i]+wsRan[watershed[i]]+
                          betaFlood*flood[i],
                        tau.site) #variation for random site effect
    
    #trend split out such that trendMu equals the main effect of year and others represent interactions with year
    #the fixed effects are indexed at the site level
    
    trendSite[i] <-trendMu[1]+betaTrendBasa*basa[i]+betaTrendGran*gran[i]+
      elev[i]*(betaTrendElev+betaTrendElevBasa*basa[i]+betaTrendElevGran*gran[i])+
      wsArea[i]*(betaTrendWsArea+betaTrendWsAreaBasa*basa[i]+betaTrendWsAreaGran*gran[i]) +
      flood[i]*betaTrendFlood + #end of fixed effects
