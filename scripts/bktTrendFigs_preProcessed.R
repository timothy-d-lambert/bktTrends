...
source("scripts/prepDataTrend2022_preProcessed.R")

# outFiles<-paste0("results/realTrendOutCov2Year1996Start",c(0,1),".rds")
outFiles <- paste0("results/realTrendOutNoCov.0.rds") # TEMPORARY -- need to streamline model naming across files
...
#### TIM'S SIMPLE VERSIONS START HERE ####

#### YOY temporal trend ####
out<-readRDS(outFiles[1]) # USER INPUT: Select YOY
            
plot(NA,
     xlim = range(n$year,na.rm=T), ylim = c(0,125),
     xlab = "", 
     ylab = bquote(Predicted~Adult~Brook~Trout~Abundance~(per~100~m)))
...
#   }
# }
out<-readRDS(outFiles[1]) # TO DO: CHANGE BACK TO outFiles[2] AFTER GETTING THOSE RESULTS RUN!
...
pred <- out$sims.list$mu + jagsData$year %o% out$sims.list$trendMu
...
#### ORIGINAL PLOTS (FROM EVAN'S MANUSCRIPT) BELOW HERE ####
